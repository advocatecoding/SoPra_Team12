...
class PersonMapper(Mapper):
    ...
    def find_all(self):
        ...
        for (person_id, vorname, nachname,  mail_adresse, benutzername, urlaubstage, ueberstunden, letzte_aenderung, user_id) in person_daten:
            person = Person()
    ...
    def find_by_id(self, id):
        ...
        try:
            (person_id, vorname, nachname,  mail_adresse, benutzername, urlaubstage, ueberstunden, letzte_aenderung, user_id) = person_daten[0]
            person = Person()
            person.set_vorname(vorname)
            person.set_nachname(nachname)
            person.set_benutzername(benutzername)
    ...
    def insert(self, person):
        ...
        command = "INSERT INTO Person (person_id, vorname, nachname, mail_adresse, benutzername, urlaubstage, ueberstunden, letzte_aenderung, google_user_id) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        data = (person.get_id(), person.get_vorname(), person.get_nachname(), person.get_mail_adresse(), person.get_benutzername(), person.get_urlaubstage(), person.get_ueberstunden(), person.get_letzte_aenderung(), person.get_user_id)
    ...
    def update(self, person):
        cursor = self._cnx.cursor()
        command = "UPDATE Person " + "SET vorname=%s, nachname=%s, mail_adresse=%s, urlaubstage=%s, letzte_aenderung=%s, user_id=%s WHERE person_id=%s"
        data = (person.get_vorname(), person.get_nachname(), person.get_mail_adresse(), person.get_urlaubstage(), person.get_letzte_aenderung(), person.get_user_id(),
                person.get_id())
        cursor.execute(command, data)
